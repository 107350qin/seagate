...
<div class="nav">
    ...
    <a href="${pageContext.request.contextPath}/user/marketActivity" style="background-color:#00C4C4;">市场活动</a>
...
<div class="all">
    <div class="left">
        <a style="background-color: rgb(210, 248, 235);
        ">市场活动</a>
        <a href="${pageContext.request.contextPath}/user/incentivePlan">奖励计划</a>
    </div>
    <div class="right">
        <table  cellspacing="0"  style="width:98.5%">
            <tr style="background-color:#F2F2F2">
                <th style="text-align: left;font-weight: bold;font-size: 18px">市场活动</th>
                <th></th>
         ...
         <c:forEach var="p" items="${pager.content}">
		    <tr>
                <td style="width:65%;overflow: hidden;"><a href=""> ${p.event_title}</a></td>
                <td>${p.event_time}</td>
                <td style="width:10%;">${p.event_browCount}</td>
            </tr>
         </c:forEach>
    ...
    <c:if test="${pager.pageNow>1}">
     <a href="${pageContext.request.contextPath}/user/marketActivity?pageNow=${pager.pageNow-1}" style="text-align:center;line-height:30px;margin-left:10px;display:block;float:left;width:80px;height:30px;background-color:yellow;">
    		上一页
    </a>
    </c:if>
    ...
    <c:if test="${pager.pageNow!=i}">
    <a href="${pageContext.request.contextPath}/user/marketActivity?pageNow=${ i }" style="text-align:center;line-height:30px;margin-left:10px;display:block;float:left;width:30px;height:30px;background-color:red;">
    		${ i }
    </a>
    </c:if>
