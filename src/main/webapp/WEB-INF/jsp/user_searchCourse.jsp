...
<div class="nav">
        ...
        <caption style="font-size:20px;text-align:left;">
            ...
            <c:forEach var="course" items="${pager.content}">
            	<tr class="btr">
	                ...
	                <td class="ctd"><a href="${pageContext.request.contextPath}/user/enterStudy?c_id=${course.c_id}">查看内容</a></td>
	                <td class="dtd">
	                    <dl>
	                        <dt>课程简介:</dt>
	                        <dd>${course.c_desc}</dd>
    ...
    <c:if test="${pager.pageNow>1}">
     <a href="${pageContext.request.contextPath}/user/search?pageNow=${pager.pageNow-1}&keyword=${keyword}&flag=1" style="text-align:center;line-height:30px;margin-left:10px;display:block;float:left;width:80px;height:30px;background-color:yellow;">
    		上一页
    </a>
    </c:if>
    ...
    <c:if test="${pager.pageNow!=i}">
    <a href="${pageContext.request.contextPath}/user/search?pageNow=${ i }&keyword=${keyword}&flag=1" style="text-align:center;line-height:30px;margin-left:10px;display:block;float:left;width:30px;height:30px;background-color:red;">
    		${ i }
    </a>
    </c:if>
